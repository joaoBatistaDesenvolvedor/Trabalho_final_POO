...
public class TelaSeries extends JFrame {
	...
	private void initialize() {
		...
		btn_cadastrar_Serie.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				try {
					...
					JOptionPane.showMessageDialog(btn_cadastrar_Serie, "Nome: " + series.getNome(),
							"serie cadastrado com sucesso", JOptionPane.INFORMATION_MESSAGE);
					...
					JOptionPane.showMessageDialog(btn_cadastrar_Serie, this, "Nao e possivel cadastrar o SERIE", 0);
					e.printStackTrace();
				}
		...
		btnBuscarSerie.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				try {
					series = IF.buscarAnime(text_nome_serie.getText());
					JOptionPane.showMessageDialog(btnBuscarSerie,
							"nome" + series.getNome() + "\n" + "genero " + series.getGenero() + "\n" + "numero ep "
									+ series.getNumero_total_ep() + "\n" + "numero temp " + series.getNumero_temporada()
									+ "\n" + "duracao " + series.getDuracao() + "\n" + "sinopse " + series.getSinopse()
									+ "\n" + "ano " + series.getAno());
				} catch (Exception e) {
					JOptionPane.showMessageDialog(btnBuscarSerie,
							"serie " + text_nome_serie.getText() + " nao existe na base de dados");
					e.printStackTrace();
					e.printStackTrace();
				}
		...
		btnCancelar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				if (text_nome_serie.getText().length() <= 0 && text_duracao_serie.getText().length() <= 0
					...
					JOptionPane.showMessageDialog(btn_cadastrar_Serie, "os campos ja estao vazios", "Cancelar Erro",
			...
			public void actionPerformed(ActionEvent arg0) {
				try {
					...
					JOptionPane.showMessageDialog(btnExcluir,
							"serie " + text_nome_serie.getText() + "excluido com sucesso");
				} catch (Exception e) {
					JOptionPane.showMessageDialog(btnExcluir,
							"Serie " + text_nome_serie.getText() + "nao possivel escluir");
					e.printStackTrace();
				}
			}
		});
